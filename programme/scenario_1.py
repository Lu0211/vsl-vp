...
from ccparams import GOING_TO_POSITION, COLLISION, JOIN, SPEED, DISTANCE
...
def main(demo_mode, real_engine, setter=None):
    global accident_lane, leader_id, platoon_lane
    ...
    while running(demo_mode, step, 6000):
        ...
        if step == 0:
            ...
            traci.vehicle.add(vehID = 'v.0.0', routeID = "platoon_route", typeID = "vtypeauto",
                              departPos = str(200), departLane = str(0), departSpeed = str(20.00))
            traci.vehicle.add(vehID = 'v.0.1', routeID = "platoon_route", typeID = "vtypeauto",
                              departPos = str(50), departLane = str(1), departSpeed = str(18.00))
        ...
        #                       departPos=str(20), departLane = str(0), departSpeed=str(30.00))

        # 发生瓶颈问题时选择车排车道和排长
        if step == 200:
            # 产生瓶颈问题
            accident_lane = create_accident(vehicle_num = 3)
        ...
        if step == 220:
            ...
            # ?没办法更改vehID，只能将排长那辆车删除再添加，或者重写数据结构去定义vehID
            # traci.vehicle.remove(vehID=leader_id)
            # print(traci.vehicle.getIDList())

        if step > 250:
            topology = Join_platoon(plexe, join_id = 'v.0.1', leader_id = leader_id,
                         front_id = leader_id, topology = topology,step = step, begin_join = 300)
