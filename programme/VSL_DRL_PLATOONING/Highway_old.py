...
class Highway():
    def __init__(self):
        ...
        self.epsilon_decay = 0.95  # 减少率：epsilon会随着迭代而更新，每次会乘以0.995
        self.epsilon_min = 0.01  # 小于最小epsilon就不再减少了。

        # 其余超参数
        self.memory = deque(maxlen=20000)  # 队列，最大值是2000
        self.batch = 256
        self.gamma = 0.97  # 折扣率
    ...
    def get_platoon_lane(self):
        ...
        # # 设置好车排车道
        for x in range(3):
            platoon_lane_id = lane_ids[x] + '_' + str(platoon_lane)
    ...
    def get_state(self, vids, leader_id):
        ...
        state = [[int(leader_id[4]),
                  int(0),
                  traci.vehicle.getPosition(leader_id)[0],
        ...
        for id in vids:
            ...
            s = [int(id[4]), type, pos[0], lane_index, speed]
            state.append(s)
    ...
    def perform_action(self, agent_vids, action, leader_id):
        ...
        for a in action_list:
            if a[1] == 1:
                ...
                self.join_state[vid] = {'state': v_state, 'front_id': fid, 'leader_id': leader_id}
    ...
    def Is_joined_platoon(self):
        ...
        """
        if self.join_state:
            for i in self.join_state.keys():
                v_state = self.join_state[i]['state']
                fid = self.join_state[i]['front_id']
                if v_state == GOING_TO_POSITION:
                    if get_distance(self.plexe, i, fid) < DISTANCE * 2:
                        print('距离够了完成合并')
                        self.plexe.set_path_cacc_parameters(i, distance=5)
                        self.plexe.set_active_controller(i, CACC)
                        self.join_state[i]['state'] = COMPLETED
                        # 检测leader是否需要恢复速度
